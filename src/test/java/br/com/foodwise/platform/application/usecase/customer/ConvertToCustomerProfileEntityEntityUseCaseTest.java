...
class ConvertToCustomerProfileEntityEntityUseCaseTest {
    ...
    void shouldReturnCustomerProfileWhenConversionIsSuccessful() {
        ...
        CustomerProfileEntity expectedCustomer = Instancio.create(CustomerProfileEntity.class);

        when(customerProfileRequestToEntityConverter.convert(request)).thenReturn(expectedCustomer);

        CustomerProfileEntity result = convertToCustomerProfileEntityUseCase.execute(request);
